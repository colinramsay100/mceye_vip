...
'LD.Canvas.DrawCircle(G.GI.originX + G.GI.intervalX * (i - 1), calcPointToPixel(py2(a), G),10,Colors.Green,True,5)
	...
	Else
		...
		fst=True
		py = point.Y
		For i = Main.Xstart To LD.Points.Size - 1
			If fst Then
				fst=False
				point = LD.Points.Get(i)
				LD.Canvas.DrawLine(G.GI.originX + G.GI.intervalX * i, calcPointToPixel(point.Y, G) _
			...
			Else	
				...
				LD.Canvas.DrawLine(G.GI.originX + G.GI.intervalX * (i - 1), calcPointToPixel(py, G) _
					, G.GI.originX + G.GI.intervalX * i, calcPointToPixel(point.Y, G), LD.LinesColors.Get(0), 2dip)
	'			LD.Canvas.DrawCircle(G.GI.originX + G.GI.intervalX * (i - 1), calcPointToPixel(py, G),4,Colors.Black,True,5)
				py = point.Y
			End If
		Next
		' plot points on graph
'		Main.numsets
		Dim m,n,PointCol As Int
		Dim Fill As Boolean
		Fill=True 
		m=0
		Do While m<Main.NumSets
			If m=0 Then 
				PointCol=Colors.Blue
			Else
				PointCol=Colors.Green
			End If
			n=0
			Do While Main.DataXY(m,n,0)<>-1
				LD.Canvas.DrawCircle(G.GI.originX + G.GI.intervalX * Main.DataXY(m,n,0)/Main.Xstep,calcPointToPixel(Main.DataXY(m,n,1), G),6,PointCol,Fill,2)
				n=n+1
			Loop
			m=m+1
		Loop
	End If
	Main.NumSets=LD.Points.Size
